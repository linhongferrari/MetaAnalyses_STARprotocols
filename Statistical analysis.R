...
#OR calculation
ORresult<-OR(form1,meta.d,clinic.d,outcome.d)
write.csv(ORresult,"OR.csv")
#split dataset to 2 sub datasets
split.d<-split_dataset(1,0.5)
t_ORresult<-OR(form1,split.d[[1]],split.d[[2]],split.d[[3]])
write.csv(t_ORresult,"subdataset1_OR.csv")
v_ORresult<-OR(form1,split.d[[4]],split.d[[5]],split.d[[6]])
write.csv(v_ORresult,"subdataset2_OR.csv")
